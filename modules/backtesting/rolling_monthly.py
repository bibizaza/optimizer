...
def rolling_backtest_monthly_param_sharpe(
...
)->tuple[pd.Series, np.ndarray, np.ndarray, pd.Timestamp]:
    """
    1) each 'months_interval' months => gather trailing 'window_days' => call param_sharpe_fn => get w
    2) apply trade buffer => transaction cost => track daily portfolio val
    """
    df_prices=df_prices.sort_index().loc[start_date:end_date]
    if len(df_prices)<2:
        empty_line=pd.Series([1.0],index=df_prices.index[:1],name="Rolling_Ptf")
        return empty_line, np.zeros(df_prices.shape[1]), np.zeros(df_prices.shape[1]), None
    ...
    for d in range(1,n_days):
        ...
        if day in rebal_dates and d>0:
            ...
            w_adj=apply_trade_buffer(
                old_w=old_w,
                new_w=w_opt,
                buffer_thr=trade_buffer_pct
            )
            cost=compute_transaction_cost(
                rolling_val, old_w, w_adj,
    ...
    return sr_norm, last_w_final, final_old_w_last_rebal, final_rebal_date