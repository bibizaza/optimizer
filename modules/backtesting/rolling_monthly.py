...
from modules.backtesting.trade_buffer import apply_trade_buffer, build_min_instrument_weights_array
...
def shift_to_valid_day(date: pd.Timestamp, valid_idx: pd.Index) -> pd.Timestamp:
    """If 'date' is not in valid_idx, return the next valid date or the last valid date."""
...
def build_monthly_rebal_dates(start_date: pd.Timestamp,
    ...
    """
    Construct a list of rebalancing dates between start_date and end_date,
    stepping by 'months_interval' months, each time taking the last day of the month.
    Then shift it to a valid date in df_prices.
    """
    rebal_dates = []
...
def compute_transaction_cost(curr_val: float,
    ...
    """
    Compute the transaction cost given either a percentage (turnover-based)
    or a fixed ticket fee per instrument traded.
    """
    ...
    if tx_cost_type == "percentage":
        ...
        cost = inst_traded * tx_cost_value
    return cost
...
def rolling_backtest_monthly_param_sharpe(
    ...
    """
    Perform a monthly rolling backtest:
      1) Each 'months_interval' months, gather the trailing 'window_days' returns
         and call param_sharpe_fn(...) to get an optimized weight vector.
      2) Apply a "trade buffer" and any transaction costs.
      3) Track daily portfolio value until the next rebalance.

    Returns:
      - sr_norm (pd.Series): normalized daily portfolio value
      - last_w_final (np.ndarray): the final optimized weight from the last rebalance
      - final_old_w_last_rebal (np.ndarray): old distribution just before the last rebalance
      - final_rebal_date (pd.Timestamp): the final rebalance date
      - df_rebal_history (pd.DataFrame): table of each rebalance (old/new weights, tx cost, etc.)
    """
    ...
    # Initialize portfolio => equal weighting among valid instruments
    rolling_val = 1.0
    ...
    final_old_w_last_rebal = np.zeros(n_assets)
    final_rebal_date = None

    # We'll store each rebalance event here
    rebalance_events = []
    ...
    for d in range(1, n_days):
        ...
        if day in rebal_dates and d > 0:
            ...
            final_old_w_last_rebal = old_w.copy()
            final_rebal_date = day

            # sub-window of returns => trailing window_days
            start_idx = max(0, d - window_days)
            sub_ret = df_returns.iloc[start_idx:d]

            # Solve param_sharpe => get new weights
            w_opt, _ = param_sharpe_fn(sub_ret)

            # trade buffer
            w_adj = apply_trade_buffer(old_w, w_opt, buffer_thr=trade_buffer_pct)
            ...
            old_val_before = rolling_val
            rolling_val -= cost
            if rolling_val < 0:
                rolling_val = 0.0

            # re-allocate (buy new shares)
            ...
            # Store debug info for this rebalance
            rebalance_events.append({
                "Date": day,
                "OldWeights": old_w.copy(),
                "NewWeights": w_adj.copy(),
                "TxCost": cost,
                "PortValBefore": old_val_before,
                "PortValAfter": rolling_val
            })

    # Build final timeseries, normalized
    ...
    # Build a DataFrame of rebalance events
    df_rebal_history = pd.DataFrame(rebalance_events)

    return sr_norm, last_w_final, final_old_w_last_rebal, final_rebal_date, df_rebal_history
