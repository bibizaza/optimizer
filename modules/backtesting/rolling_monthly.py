...
def compute_transaction_cost(
    ...
    """
    Calculates transaction cost given:
      - current portfolio value (curr_val)
      - old weights (old_w)
      - new weights (new_w)
      - transaction cost amount (tx_cost_value)
      - cost type = 'percentage' or 'ticket_fee'
    """
...
def compute_cost_impact(
    ...
    relative to the final portfolio value => fraction of final wealth.
    """
...
def rolling_backtest_monthly_param_sharpe(
    ...
    Rolls monthly between start_date and end_date:
      1) At each rebalance date, subsets returns for the last 'window_days'
         but keeps the same columns (tickers).
      2) Calls 'param_sharpe_fn(sub_ret)' => new weights
      3) Applies transaction cost, updates portfolio shares
      4) Returns final portfolio series, rebal info, etc.
    """

    df_prices = df_prices.sort_index()
    df_prices = df_prices.loc[start_date:end_date]
    # Fill missing so columns remain the same
    ...
    for d in range(1, n_days):
        ...
        if day in rebal_dates:
            ...
            sub_ret = df_returns.iloc[start_idx:d].copy()

            # param_sharpe_fn => new weights
            w_opt, _ = param_sharpe_fn(sub_ret)

            # cost
            cost = compute_transaction_cost(
                rolling_val, old_w, w_opt,
